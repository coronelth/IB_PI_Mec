...
La instrucción de asignar opciones de compilación en \textsc{C} y \textsc{Cuda C} son respectivamente las siguientes:
\\\\
...
sea la que el compilador utilice
distintas del código, la primera es para que el código tenga las variables mencionadas y la segunda es para que el compilador sepa cuál de ellas realizar.

Comenzando 


por lo que es necesario realizar una instrucción para que CMake al compilar detecte qué tipo de variable se desea utilizar.  La forma de obtener lo planteado es crear una variable y en base a su contenido, el código realizado denotará los tipos de variables en simple precisión (\textit{float}) o doble precisión (\textit{double}).
...
ella 
Por último se realizó la instrucción para que el código pueda compilarse con variables de tipo simple precisión y doble precisión, donde se puede seleccionar de manera discriminada la precisión en cada lenguaje realizado. Para que de forma automática se realice ésto en el proyecto, además de las opciones de compilación que se detallan en el siguiente \textit{script}, se debe realizar un \textit{header} en el proyecto para que se incluya el mismo, dicho \textit{header} en el código es el que tiene el nombre dataTypes.h y debe ser incluido en cada una de las funciones del código que se realicen.
