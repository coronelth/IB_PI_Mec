...
El parámetro \textit{A}  ($\frac{atm L^2}{mol^2}$) caracteriza la interacción que poseen las moléculas del gas entre sí, y \textit{B} ($\frac{L}{mol}$) da una idea del volumen molar mínimo que posee una partícula del fluido (este parámetro define a la partícula del gas con un dado volúmen en vez de ser puntual como en la Ley de gases ideales).
...
Para el modelo de grilla D2Q9 utilizado en el presente trabajo $\mathbf{m}_{eq}$, $ \Lambda$ y $\mathbf{M}$ están dados por:
\begin{align}
...
Se pueden recuperar las ecuaciones de Navier - Stokes utilizando Ecs. [\ref{eq:rho} - \ref{eq:termino_fuente_s} ] mediante el análisas de Chapman-Enskog limitado para un número de Mach bajo \cite{li2013lattice}, donde las ecuaciones recuperadas son \cite{fogliatto2019simulation} \cite{li2013lattice}:
