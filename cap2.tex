...
Primero se calcula el valor de la distribución de equilibrio del nodo $i$-ésimo $\mathbf{g^{eq}}$ utilizando Ec. (\ref{eq:n_eq}) y $\mathbf{M}$ de la forma $\mathbf{g^{eq}} = \mathbf{M}^{-1} \mathbf{n^{eq}}$. Se prosigue a calcular el parámetro $\beta$:
